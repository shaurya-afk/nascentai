...
export default function GitHubLogoutButton({
  ...
  className = "",
  disabled = false,
}: GitHubLogoutButtonProps) {
  return (
    <button
      type="button"
      onClick={onClick}
      disabled={disabled}
      className={`inline-flex items-center justify-center gap-3 rounded-lg border border-border bg-surface px-6 py-2.5 text-sm font-medium text-white transition-colors hover:bg-hover active:bg-neutral-900 disabled:cursor-not-allowed disabled:opacity-50 ${className}`}
    >
      <svg
        viewBox="0 0 24 24"
        fill="currentColor"
